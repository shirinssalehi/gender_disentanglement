...
# from sentence_transformers.cross_encoder import DisentangledCrossEncoder
...
with open(train_filepath, 'rt') as fIn,\
    open("../data/triples_2e6_msmarco.tsv", "w") as file_to_write:
    for line in tqdm.tqdm(fIn, unit_scale=True):
        ...
        if (cnt % (pos_neg_ration+1)) == 0:
            passage_pos = corpus[pos_id]
        else:
            passage_neg = corpus[neg_id]
        cnt += 1
        file_to_write.write(query+"\t"+passage_pos+"\t"+passage_neg+"\n")
