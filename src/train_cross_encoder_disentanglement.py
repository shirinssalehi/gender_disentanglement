...
from sentence_transformers.cross_encoder import OriginalCrossEncoder
# from sentence_transformers.cross_encoder.evaluation import CERerankingEvaluator
from sentence_transformers import InputExampleDisentanglement
import logging
...
if disen:
    model_save_path = '../checkpoints/disentangled/llm_experiments/6M_'+model_name.replace("/", "-")
    model = DisentangledCrossEncoderAdv(model_name, num_labels=1, max_length=512)
else:
    model_save_path = '../checkpoints/original/llm_experiments/6M_'+model_name.replace("/", "-")
    model = OriginalCrossEncoder(model_name, num_labels=1, max_length=512)
...
# train_filepath = "/home/sara-salamat/Projects/Shirin_work/datasets/Dataset-genderlabelled-1neg.tsv"
train_filepath = "../data/balanced_triples_genderlabelled_passage_label.tsv"
...
with open(train_filepath, 'rt') as fIn:
    for line in tqdm.tqdm(fIn, unit_scale=True):
        ...
        relevance_label = 1
        gender_label = int(gender_pos)
        train_samples.append(InputExampleDisentanglement(texts=[query, passage], label={"relevance_label":relevance_label, "gender_label": gender_label}))

        # else:
        passage = corpus[neg_id]
        relevance_label = 0
        gender_label = int(gender_neg)
        train_samples.append(InputExampleDisentanglement(texts=[query, passage], label={"relevance_label":relevance_label, "gender_label": gender_label}))
        cnt += 2
