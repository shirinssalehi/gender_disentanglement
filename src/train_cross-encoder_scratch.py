...
from sentence_transformers.cross_encoder import CrossEncoderScratch
...
train_batch_size = 32
num_epochs = 1
model_save_path = '../checkpoints/scratch_triples_genderlabelled_passage_label_2e6_'+model_name.replace("/", "-")+'-'+datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
...
model = CrossEncoderScratch(model_name, num_labels=1, max_length=512)
...
with open(train_filepath, 'rt') as fIn:
    for line in tqdm.tqdm(fIn, unit_scale=True):
        qid, pos_id, neg_id, _, _ = line.strip().split()
